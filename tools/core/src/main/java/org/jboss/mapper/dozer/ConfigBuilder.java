...
public class ConfigBuilder {
    ...
    // be called once.
    public Mapping addClassMapping(final String fromClass, final String toClass) {
    ...
    public List<MappingOperation<?,?>> getMappings() {
        ...
        for (Mapping m : mapConfig.getMapping()) {
            for (Object o : m.getFieldOrFieldExclude()) {
                if (!(o instanceof Field)) {
                    continue;
                }
                Field f = (Field)o;
                if (LITERAL_MAPPER_CLASS.equals(f.getCustomConverter())) {
                    mappings.add(new LiteralMapping(
                            new Literal(f.getCustomConverterParam()),
                            f.getB().getContent()));
                } else {
                    mappings.add(new FieldMapping(
                            f.getA().getContent(), f.getB().getContent()));
                }
            }
        }
        return mappings;
    ...
    public void map(final Literal literal, final Model target) {
    ...
    Mapping getLiteralMapping(final Model target) {
        ...
        if (mapping == null) {
            mapping = addClassMapping(LITERAL_MAPPER_CLASS, target.getParent().getType());
        }
