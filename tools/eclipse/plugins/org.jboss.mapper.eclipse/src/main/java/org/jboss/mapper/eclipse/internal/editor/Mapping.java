...
final class Mapping {
    ...
    Mapping( final TransformationEditor editor,
        ...
        mappingSourcePane.setLayout( GridLayoutFactory.swtDefaults().spacing( 0, 0 ).numColumns( 4 ).create() );
        ...
        opImage = customMapping ? Util.DELETE_IMAGE : fieldMapping ? Util.ADD_OPERATION_IMAGE : null;
        ...
        if ( customMapping ) {
            ...
            opStartLabel.setToolTipText( cMapping.getMappingClass() + '.' + cMapping.getMappingOperation() );
        }
        ...
        if ( mappingsViewer.textBackground == null ) mappingsViewer.textBackground = sourceText.getBackground();
        ...
        final TraversalListener sourceTraversalListener = new TraversalListener( mappingsViewer.prevTargetText, targetText );
        ...
        if ( mappingsViewer.prevTraversalListener != null ) mappingsViewer.prevTraversalListener.nextText = sourceText;
        ...
        opIconLabel.addMouseListener( new MouseAdapter() {
            ...
            public void mouseUp( final MouseEvent event ) {
                try {
                    if ( opImage == Util.ADD_OPERATION_IMAGE ) addCustomOperation( mouseOverListener );
                    else if ( opImage == Util.DELETE_IMAGE ) removeCustomOperation();
                } catch ( final Exception e ) {
                    Activator.error( e );
                }
            }
        ...
        sourceDropTarget.addDropListener( new DropListener( sourceText, editor.sourceModel(), mappingSourcePane ) {
        ...
        targetDropTarget.addDropListener( new DropListener( targetText, editor.targetModel(), mappingTargetPane ) {
    ...
    void addCustomOperation( final MouseTrackListener mouseOverListener ) throws Exception {
        ...
        final AddCustomOperationDialog dlg = new AddCustomOperationDialog( mappingsViewer.getShell(),
                                                                           editor.project(),
                                                                           mapping.getSource().getType() );
        if ( dlg.open() != Window.OK ) return;
        this.mapping = editor.map( mapping, dlg.type.getFullyQualifiedName(), dlg.method.getElementName() );
        opStartLabel.setText( dlg.method.getElementName() + "(" );
        opStartLabel.setToolTipText( dlg.type.getFullyQualifiedName() + '.' + dlg.method.getElementName() );
    ...
    void update() {
        ...
        mappingsViewer.scroller.setMinSize( mappingsViewer.pane.computeSize( SWT.DEFAULT, SWT.DEFAULT ) );
    }
    ...
    private abstract class DropListener extends DropTargetAdapter {
        ...
        public final void dragEnter( final DropTargetEvent event ) {
            if ( valid( dragSource() ) ) dropText.setBackground( mappingPane.getDisplay().getSystemColor( SWT.COLOR_BLUE ) );
        }
